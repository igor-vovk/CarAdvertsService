...
class RepositorySpec extends FlatSpec with MustMatchers with Inspectors with OptionValues with ScalaFutures with OneAppPerSuite {

  implicit val cfg = PatienceConfig(scaled(Span(10, Seconds)))

  def repositoryTest(repository: CarAdvertsRepository): Unit = {
  ...
  "Test repository" must behave like repositoryTest(new CarAdvertsRepositoryBackedBySeq())
  "Dynamo repository" must behave like repositoryTest(app.injector.instanceOf[CarAdvertsRepositoryDynamoProvider].get())
